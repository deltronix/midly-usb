use crate::buffer::*;
use crate::io::{NotSeekable, Write, WriteResult};
use crate::packet::*;
use crate::{class::*, packet};
...
pub struct MidiDevice<'a, B, RS = DefaultBufferStore, WS = DefaultBufferStore>
where
    B: UsbBus,
    RS: BorrowMut<[u8]>,
    WS: BorrowMut<[u8]>,
    ...
    B: UsbBus,
{
    ...
    ) -> MidiDevice<'_, B, DefaultBufferStore, DefaultBufferStore> {
        MidiDevice::new_with_store(alloc, unsafe { mem::uninitialized() }, unsafe {
            mem::uninitialized()
        })
    }
}

impl<B, RS, WS> MidiDevice<'_, B, RS, WS>
where
    B: UsbBus,
    RS: BorrowMut<[u8]>,
    WS: BorrowMut<[u8]>,
{
    ...
    pub fn new_with_store(
        ...
        read_store: RS,
        write_store: WS,
    ) -> MidiDevice<'_, B, RS, WS> {
        MidiDevice {
            inner: MidiClass::new(alloc, 64),
...
impl<B, RS, WS> UsbClass<B> for MidiDevice<'_, B, RS, WS>
where
    B: UsbBus,
    RS: BorrowMut<[u8]>,
    WS: BorrowMut<[u8]>,
...
impl<B, RS, WS> Write for MidiDevice<'_, B, RS, WS>
where
    B: UsbBus,
    RS: BorrowMut<[u8]>,
    WS: BorrowMut<[u8]>,
{
    ...
    fn write(&mut self, data: &[u8]) -> WriteResult<Self> {
        let count = self.write_buf.write(data);
...
// impl<B, RS, WS> embedded_hal::serial::Write<u8> for MidiDevice<'_, B, RS, WS>
// where
//     B: UsbBus,
//     RS: BorrowMut<[u8]>,
//     WS: BorrowMut<[u8]>,
...
//         match <MidiDevice<'_, B, RS, WS>>::write(self, slice::from_ref(&word)) {
...
//         match <MidiDevice<'_, B, RS, WS>>::flush(self) {
...
// }
//
// impl<B, RS, WS> embedded_hal::serial::Read<u8> for MidiDevice<'_, B, RS, WS>
// where
//     B: UsbBus,
//     RS: BorrowMut<[u8]>,
//     WS: BorrowMut<[u8]>,
...
//         let mut buf: u8 = 0;
//
//         match <MidiDevice<'_, B, RS, WS>>::read(self, slice::from_mut(&mut buf)) {
//             Ok(0) | Err(UsbError::WouldBlock) => Err(nb::Error::WouldBlock),
//             Ok(_) => Ok(buf),
//             Err(err) => Err(nb::Error::Other(err)),
//         }
